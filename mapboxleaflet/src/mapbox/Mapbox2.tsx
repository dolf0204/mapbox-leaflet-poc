...
export const Mapbox2: FC = () => {
  ...
  const [timer, setTimer] = useState({ counter: 0 });

  const [thetas, setThetas] = useState<number[]>([]);

  const [viewportStateLatitude, setViewportStateLatitude] = useState(0);
  const [viewportStateLongitude, setViewportStateLongitude] = useState(0);
  const [viewportStateZoom, setviewportStateZoom] = useState(0);
  const [geojson, setGeojson] = useState<
    GeoJSON.FeatureCollection<GeoJSON.Geometry>
  >(initialFeautureCollection as any);
  ...
  const setMapRef = (map: any) => {
    if (map) {
      ...
      setMapState(mapGL);
      const viewPortLatitude = Math.round(viewport.latitude * 3) / 3;

      const viewPortLongitude = Math.round(viewport.longitude * 3) / 3;

      const viewportZoom = Math.round(viewport.zoom * 2) / 2;

      if (
        viewPortLongitude !== viewportStateLongitude ||
        viewPortLatitude !== viewportStateLatitude ||
        viewportZoom !== viewportStateZoom
      ) {
        setViewportStateLongitude(viewPortLongitude);
        setViewportStateLatitude(viewPortLatitude);
        setviewportStateZoom(viewportZoom);
      }
    }
  };
    ...
    const circlesUpdater = setInterval(() => {
      if (mapState) {
        for (let i = 0; i < thetas.length; i++) {
          ...
          const newLng: number =
            ll[0] + (dy / radEarth) * (degFactor / Math.PI);
          const newLat: number =
            ll[1] +
        ...
        //   (geojson.features[0].geometry as any).coordinates.push(
        //     getRandomLatLng()
        //   );
        // }
        ...
        if (mapState && geojson) {
          ...
          //   (mapState.getSource("circlesSet") as mapboxgl.GeoJSONSource).setData(
          //     geojson
          //   );
          // mapState.addLayer({
          //     id: "circle"+newFeature.geometry.coordinates[0],
          //     type: "circle",
          //     source: "waypoints",
          //     // "layout": {
          //     //     "visibility": "none"
          ...
          // });
          id++;
          if (mapState.getSource("circlesSet")) {
            mapState.removeLayer(`circles-${thetas.length - 1}`);

            mapState.removeSource("circlesSet");
          }

          mapState.addSource("circlesSet", {
            type: "geojson",
            data: geojson,
          });
          debugger;
          mapState.addLayer({
            id: `circles-${thetas.length}`,
            type: "circle",
            source: "circlesSet",
            paint: {
              "circle-radius": 6,
  ...
  return (
    <div className="mapbox-container" id="map">
      <MapGL
        ...
        onLoad={(event: any) => onLoadHandler(event)}
